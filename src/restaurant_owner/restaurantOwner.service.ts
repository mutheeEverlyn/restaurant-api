...
export const  restaurantOwnerService = async (limit?: number):Promise<tsRestaurantOwner[] | unknown> => {
    if (limit) {
...
export const getRestaurantOwnerService = async (id: number):Promise<tsRestaurantOwner[] | unknown> => {
    return await db.query.tableRestaurantOwner.findFirst({
        where: eq(tableRestaurantOwner.id, id)
    })
}
//between
export const restaurantIdBetween= async (id1: number,id2:number):Promise<tsRestaurantOwner[] | unknown> => {
    return await db.query.tableRestaurantOwner.findMany({
        where: between(tableRestaurantOwner.id, id1,id2)
    })
}

export const createRestaurantOwnerService = async ( restaurantOwner:any):Promise<tiRestaurantOwner[] | unknown> => {
    await db.insert(tableRestaurantOwner).values( restaurantOwner)
    return " restaurantOwner created successfully";
}

export const updateRestaurantOwnerService = async (id: number,  restaurantOwner: any):Promise<tiRestaurantOwner[] | unknown> => {
