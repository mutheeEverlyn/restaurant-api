...
import { ordersService, getOrdersService, createOrdersService, updateOrdersService, deleteOrdersService,orderPrice,orderData } from "./orders.service";
...
//price >500
export const priceOrders= async (c: Context) => {
    try {
        const price = Number(c.req.query('price'))

        const data = await orderPrice(price);
        if (data == null || data.length == 0) {
            return c.text("orderPrice not found", 404)
        }
        return c.json(data, 200);
    } catch (error: any) {
        return c.json({ error: error?.message }, 400)
...
//order data
export const Orders = async (c: Context) => {
    try {
